...
void AddBookWindow::addBook()
{
    ...
    else if (!ui->lineTitle->text().isEmpty() && !ui->comboAuthor->currentText().isEmpty() && !ui->comboGenre->currentText().isEmpty()
...
void AddBookWindow::setBook(std::shared_ptr<jp::Book> book)
{
    ...
    ui->label_2->setText("Перед изменением убедитесь, что все поля заполены правильно.");
    book_ = book;
...
void AddBookWindow::reject()
{
    ...
    ui->buttonBox->button(QDialogButtonBox::Ok)->setText("Добавить");
    ui->label_2->setText("Перед добавление убедитесь, что все поля заполены правильно.");
    book_.reset();
